...
namespace StartupEd.UX.Incubation.Apps.VentureSimulation
{
    #line hidden
...
#line hidden
#nullable disable
    [Microsoft.AspNetCore.Components.RouteAttribute("/app/venture/new")]
    [Microsoft.AspNetCore.Components.RouteAttribute("/app/venture/{VentureId}")]
    public partial class VentureOLP : BaseComponent
    {
...
#nullable restore
#line 415 "/Users/ankitbuti/workspace/bigapple/UXs/Incubation/Apps/VentureSimulation/VentureOLP.razor"
    ...
    {
        module = await JSRuntime.InvokeAsync<IJSObjectReference>("import", "./assets/js/wizards/venture_wizard.js");
        await module.InvokeVoidAsync("Wizard", "ventureWizard","ventureForm");
    }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (VentureId != null)
        {
            Model = await SimulatorService.GetSimulatorById(VentureId);
        }
    }
    ...
    public async Task SubmitModel()
    {
        try
        {
            ...
            number++;
            if (VentureId != null && VentureId != "new")
            {
                response = await SimulatorService.PutAsync(Model);
                if (response.IsSuccessStatusCode)
                {
                    ToastService.ShowSuccess("Simulator Updated Successfully!", "");
                }
            }
            else
            {
                Model.Name = "Simulator-" + number;
                response = await SimulatorService.PostAsync(Model);
                if (response.IsSuccessStatusCode)
                {
                    ToastService.ShowSuccess("Simulator Created Successfully!", "");
                }
                else
                {
            ...
            navigationManager.NavigateTo("/app/venture");
        }
        catch (Exception ex)
        {
            SLog.Write(ex);
